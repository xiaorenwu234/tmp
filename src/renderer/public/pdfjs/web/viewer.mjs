...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    const params = (0,_ui_utils_js__WEBPACK_IMPORTED_MODULE_0__.parseQueryString)(window.location.hash.substring(window.location.hash.indexOf('?')));
    console.log(params)
    document.title = params.get('name');
    let file_url, file_key;
    try {
      const response = await instance.get('/users/get-pdf', {
        params: {
    ...
    if (file_url) {
      this.open({
        url: file_url,
        file_key,
        httpHeaders: instance.defaults.headers.common
      });
    } else {
...
class GenericL10n extends _l10n_js__WEBPACK_IMPORTED_MODULE_3__.L10n {
  ...
  static async #getPaths() {
    ...
    let paths = await (0,pdfjs_lib__WEBPACK_IMPORTED_MODULE_2__.fetchData)(href, "json");
    let baseURL = href.replace(/[^/]*$/, "") || "./";
    if (!href.startsWith("http")) {
      // in electron build
      baseURL = window.location.href.split('index')[0] + 'pdfjs/web/locale/'
    }
    return {
      baseURL,
      paths
...
class PDFPrintService {
  ...
  layout() {
    ...
    reader.setAttribute("data-pdfjsprinting", true);
    const {
      width,
      height
    ...
    reader.append(this.pageStyleSheet);
  }
  destroy() {
    ...
    const reader = document.querySelector("#reader");
    reader.removeAttribute("data-pdfjsprinting");
...
window.print = async function () {
  ...
  let res = await window.electronAPI.getPrintOption();
  if (res) {
    abort();
    return;
  }
...
function getViewerConfiguration() {
  return {
    ...
    secondaryToolbar: {
      ...
      firstPageButton: document.getElementById("secondaryFirstPage"),
      lastPageButton: document.getElementById("secondaryLastPage"),
